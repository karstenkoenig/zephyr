...
K_KERNEL_STACK_DEFINE(etr_stack, CONFIG_CORESIGHT_ETR_TBM_STACK_SIZE);
...
static void process(void)
{
	...
	while ((pending = pending_data()) >= MIN_DATA) {
		...
		if (sync_cnt || (CONFIG_CORESIGHT_ETR_TBM_SYNC_PERIOD == 0)) {
			sync_cnt--;
			...
			sync_cnt = CONFIG_CORESIGHT_ETR_TBM_SYNC_PERIOD;
...
int etr_dump_flush(bool blocking)
{
	...
	while (pending_data() >= 2 * MIN_DATA) {
		k_msleep(CONFIG_CORESIGHT_ETR_TBM_FLUSH_TIMEOUT);
	}
...
static void etr_thread_func(void *dummy1, void *dummy2, void *dummy3)
{
	while (1) {
		process();
		k_sleep(K_MSEC(CONFIG_CORESIGHT_ETR_TBM_BACKOFF));
	}
}
